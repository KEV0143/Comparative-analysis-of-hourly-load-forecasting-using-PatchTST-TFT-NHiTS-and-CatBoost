...
- *偏度与峰度：* 轻微的正偏度系数（$\text{Skewness} = 0.0896$）表明存在由极高用电时段（如负荷尖峰）引起的弱右拖尾。负峰度（$\text{Kurtosis} = -0.5303$）表明该分布为平顶态（platykurtic），即与正态分布相比，概率密度曲线更加平缓，极端异常值的出现频率较低。这与学术界的研究结论一致 [12]，即长期平均的日负荷曲线由于多个周期性用电量组的叠加，往往呈现出平滑分布；
- *波动性：* 标准差（987.74 MWh）约占均值的 21%，这表明存在显著的日内波动性。四分位距（$\text{IQR} \approx 1415$ MWh）证实大部分逐时用电量数值集中在中等负荷范围内，而极端数值（无论过低还是过高）都相对罕见。
...
&emsp;&emsp;环境温度的概率分布同样为平顶态（$\text{Kurtosis} = -0.5670$），均值（6.84 °C）与中位数（6.30 °C）接近，且具有弱负偏度（–0.0841）。这表明分布在均值周围相对对称，并略微偏向较低温度。较宽的数值分布范围（标准差 = 10.61 °C）反映了年度气候周期。环境温度概率分布图如图 2 所示。
...
&emsp;&emsp;设给定按随机置换 $\sigma$ 排列的样本集 $\{(X_k, Y_k)\}_{k=1}^n$ 和树的数量 $I$。设 $M_i(X)$ 为对象 $X$ 在第 $i$ 步训练后的模型。对所有 $i = 1, \dots, n$ 初始化 $M_i(X) = 0$。在每次迭代 $\text{iter} = 1, \dots, I$ 中，针对每个对象 $i = 1, \dots, n$，根据上一步模型的预测值计算损失函数 $L(y, a)$ 的梯度。对所有 $j = 1, \dots, i - 1$，梯度由公式 (1) 给出：
...
&emsp;&emsp;**CatBoost (Gradient Boosting).** 使用梯度提升算法作为基线方案（Baseline）。树深度（$\text{Depth} = 6$）：选择了偏差与方差之间的中间值。在现有样本上，更深的树（$\text{Depth} = 8\text{--}10$）会导致模型迅速过拟合。迭代次数（1000）：在学习率（$\text{Learning Rate}$）为 0.03 时，足以保证损失函数收敛。

&emsp;&emsp;**N-HiTS (Neural Hierarchical Interpolation).** 模型基于通过具有不同采样频率的分支（stacks）进行分层预测。池化尺寸 ([8, 4, 1]) 是核心超参数。第一个模块（$\text{kernel size} = 8$）平滑输入数据以捕捉趋势；第二个模块 (4) 对中期波动建模；第三个模块 (1) 针对原始信号工作以恢复高频细节。MLP 宽度 (512)：使用较宽的全连接层使模型能够逼近复杂的非线性依赖关系。
...
&emsp;&emsp;**残差正态分布检验 (Normality Test)。** 采用 Shapiro-Wilk 检验，结果如下：$W = 0.9871$，$p\text{-value} = 0.1249$。由于 $p\text{-value} > 0.05$（显著性水平），未拒绝残差服从正态分布的原假设。图 11 展示的 Q-Q Plot 表明残差散点几乎紧贴理论直线，同样证实了正态性。
...
&emsp;&emsp;**残差自相关检验 (Autocorrelation)。** 针对误差独立性假设，使用了 Ljung-Box Q 检验及自相关函数（ACF）分析（图 12）。结果在残差 24 小时和 48 小时滞后项上获得了 $p\text{-value} < 0.05$。因此，由于用电量复杂的日内结构未能被模型完全捕获，残差中保留了周期性依赖关系。然而，考虑到总体误差较小（$\text{MAPE } 0.68\%$）且分布正态性已获证实，残差中的自相关对于电力企业和工业企业在实际用电预测中的模型应用并不构成致命影响。
